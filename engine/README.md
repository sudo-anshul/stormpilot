...
- EPA source: [official v5.2.4](https://github.com/USEPA/Stormwater-Management-Model/tree/7952ca837988b1c32f791812eccc9fd64547e093), commit `7952ca837988b1c32f791812eccc9fd64547e093`. The upstream README states that the C source is public domain and supplies its disclaimer. Vendored source is unmodified. `native_bridge.c` reads elapsed time, native cumulative flooding, hydraulic link/node volume and signed link flow. Its explicit timestep cap preserves SWMM's adaptive Courant control, unlike the public timestep setter, which disables adaptive routing. No hydraulic equation is changed.
...
Control and fault boundaries clip the maximum routing step while preserving adaptive routing; the minimum allowed step is clipped only when needed to land on a boundary. All ablations retain the entire envelope's boundary grid. Every actual routing step is recorded, including time zero and the terminal state. `display_trace` combines five-minute samples with actual captured global discharge/overflow/storage peaks, basin-depth peaks, the first overflow and fault boundaries. It contains basin depths, held observations, command/actual valve settings and the observation timestamp. It does not interpolate missing depths.
