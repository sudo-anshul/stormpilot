...
def normalize_repair(request):
    ...
    allowed = {"controller_id", "target_scales", "balance_gains", "jump_thresholds_m", "correction_fractions", "budget"}
    if not isinstance(config, dict) or set(config)-allowed:
        raise ValueError("repair contains an unsupported search setting.")
    # Explicit legacy grids retain their original meaning; empty/default
    # requests use the causal sensor-plausibility family.
    family = config.get("controller_id", "balanced_flow" if set(config) & {"target_scales", "balance_gains"} else "plausible_depth")
    if family not in {"balanced_flow", "plausible_depth"}:
        raise ValueError("repair.controller_id must be plausible_depth or balanced_flow.")
    if family == "balanced_flow" and set(config) & {"jump_thresholds_m", "correction_fractions"}:
        raise ValueError("Sensor-plausibility arrays cannot be used for balanced_flow.")
    if family == "plausible_depth" and "balance_gains" in config:
        raise ValueError("balance_gains cannot be used for plausible_depth.")
    ...
    def values(name, default, parameter):
        ...
        normalized = [normalize_parameters(family, {parameter: item})[parameter] for item in items]
        if len(set(normalized)) != len(normalized):
            raise ValueError("Repair grid values must be unique.")
        return sorted(normalized)

    grid = {"controller_id": family, "budget": budget,
            "target_scales": values("target_scales", [1.0] if family == "plausible_depth" else [0.90, 0.94, 0.96, 0.98, 1.0], "target_scale")}
    if family == "plausible_depth":
        grid.update(jump_thresholds_m=values("jump_thresholds_m", [0.65], "jump_threshold_m"),
                    correction_fractions=values("correction_fractions", [0.25, 0.5, 0.75], "correction_fraction"))
    else:
        grid["balance_gains"] = values("balance_gains", [0.5, 1.0, 2.0, 4.0, 8.0], "balance_gain")
    return base, model, grid


def candidate_parameters(config):
    if config["controller_id"] == "plausible_depth":
        return [{"target_scale": scale, "jump_threshold_m": threshold, "correction_fraction": fraction}
                for scale, threshold, fraction in product(config["target_scales"], config["jump_thresholds_m"], config["correction_fractions"])]
    return [{"target_scale": scale, "balance_gain": gain}
...
def repair(request):
    ...
    if valid_reference:
        for parameters in candidate_parameters(config):
            if len(ledger)+4 > budget:
                break
            candidate_id = "policy-" + digest(parameters)[:12]
            first_call = len(ledger)
            runs = {subset: execute(config["controller_id"], parameters, subset, "candidate", candidate_id) for subset in subsets}
            outcome = assess_candidate(reference, runs)
            entry = {"candidate_id": candidate_id, "controller_id": config["controller_id"], "parameters": parameters,
            ...
            rank = (not outcome["eligible"], len(outcome["failures"]), runs["ab"]["metrics"]["flood_volume_m3"], tuple(parameters.values()))
            if best is None or rank < best:
                best, selected_runs, selected_parameters = rank, runs, parameters
            # Candidate parameter tuples are unique. Retain only the current
            # best proof, not every full routing trace in server memory.
            cache.clear()

    effective = dict(base)
    if selected_parameters is not None:
        effective["fallback_controller_id"] = config["controller_id"]
    ...
    grid_size = len(candidate_parameters(config))
    result["repair"] = {"schema_version": "stormpilot.repair.v1", "method": "guard-first-causal-parameter-grid-v1",
                        ...
                        "information_boundary": "Current and past measured depths and declared model geometry only; state is isolated per run. No rainfall forecast, true simulator depth, active-fault flag or heldout result reaches the controller.",
