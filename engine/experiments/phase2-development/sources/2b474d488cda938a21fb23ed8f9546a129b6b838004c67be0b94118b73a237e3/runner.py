...
INFORMATION_BOUNDARY = "causal-depth-history-feedback-v2"
...
def run_simulation(request, role="stress", controller_id=None, active_fault_ids=None, controller_parameters=None):
    ...
    with tempfile.TemporaryDirectory(prefix="swmm-", dir=runs_dir) as directory:
        ...
        try:
            ...
            def display_sample():
                result = {**sample(), "basin_depth_m": depths(), "observed_depth_m": dict(observations),
                ...
                if controller_id == "plausible_depth":
                    result["controller_diagnostics"] = {node: {"estimated_offset_m": memory["offset"],
                        "control_depth_m": memory["control_depth_m"], "detected_jumps": memory["events"],
                        "aggregate_cap_scale": memory.get("aggregate_cap_scale", 1.0)}
                        for node, memory in controller_state.items()}
                return result
            current = sample()
            ...
            while running:
                ...
                if now + 1e-7 >= next_control:
                    ...
                    held_commands = commands(controller_id, observations, model["assets"], max_depths, parameters, controller_state)
                    next_control += 300.0
