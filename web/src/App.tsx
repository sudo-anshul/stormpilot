...
import { METRICS, number, ROLE_LABELS, timeLabel } from './types';
import type { Catalog, ExperimentConfig, InvestigationView, Job, MetricId } from './types';
...
const fields: (keyof ExperimentConfig)[] = ['model_id', 'controller_id', 'fallback_controller_id', 'metric', 'threshold', 'fault_asset', 'fault_kind', 'start_hour', 'duration_hours', 'budget'];
function sameConfig(a: ExperimentConfig, b: ExperimentConfig): boolean {
  return fields.every(key => String(a[key]) === String(b[key])) && JSON.stringify(a.additional_faults ?? []) === JSON.stringify(b.additional_faults ?? []);
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([api<Catalog>('/api/catalog'), api<InvestigationView>('/api/demo')]).then(([catalogResult, demoResult]) => {
      if (cancelled) return;
      if (catalogResult.status === 'fulfilled') {
        setCatalog(catalogResult.value);
        setConfig(demoResult.status === 'fulfilled' ? { ...catalogResult.value.defaults, ...demoResult.value.config } : catalogResult.value.defaults);
      ...
      if (demoResult.status === 'fulfilled') displayView({ ...demoResult.value, recorded: true });
      setBooting(false);
    });
  ...
  async function startExperiment(replay = false) {
    ...
    try {
      const next = await api<Job>(replay && view ? `/api/jobs/${encodeURIComponent(view.id)}/replay` : '/api/jobs', {
        ...
        ...(!replay ? { body: JSON.stringify({ ...config, mode: 'investigate' }) } : {}),
      });
  ...
  const conditionName = (id: string) => { const condition = view?.search.conditions?.find(item => item.id === id); return condition ? `Outlet ${condition.asset} (${id})` : id; };
  ...
  return <div className="app-shell">
    ...
    <header className="masthead">
      ...
      <div className="masthead-right"><span className={`engine-status ${catalog?.engine.status === 'unavailable' ? 'unavailable' : ''}`}><span />{booting ? 'Connecting to solver' : catalog ? engineLabel(catalog.engine.version) : 'Solver unavailable'}</span><a className="source-link" href={view?.provenance.source_url ?? selectedModel?.source ?? 'https://www.epa.gov/water-research/storm-water-management-model-swmm'} target="_blank" rel="noreferrer">Model source <ExternalLink size={13} /></a></div>
    </header>
    ...
    <div className="workspace-layout">
        ...
        <div id="setup-content" className="setup-content">
          ...
          {booting && !config ? <div className="setup-loading"><LoaderCircle className="spin" size={20} /><p>Loading models and policies…</p></div> : config && catalog ? <form id="experiment-form" onSubmit={event => { event.preventDefault(); void startExperiment(); }}>
            <fieldset disabled={busy}>
              <div className="field"><label htmlFor="model">Model</label><select id="model" value={config.model_id} onChange={e => { const next = catalog.models.find(m => m.id === e.target.value); setConfig({ ...config, model_id: e.target.value, additional_faults: [], fault_asset: next?.asset_ids[0] ?? '', start_hour: Math.min(config.start_hour, Math.max(0, (next?.duration_hours ?? 1) - 1)), duration_hours: Math.min(config.duration_hours, next?.duration_hours ?? 1) }); }}>
                {catalog.models.map(model => <option key={model.id} value={model.id}>{model.name}</option>)}
              </select><p className="field-description">{selectedModel?.description}</p><div className="model-facts"><span>{selectedModel?.asset_ids.length ?? '—'} control assets</span><span>{number(selectedModel?.duration_hours)} h horizon</span></div></div>
              <div className="field"><label htmlFor="policy">Control policy</label><select id="policy" value={config.controller_id} onChange={e => updateConfig('controller_id', e.target.value)}>{catalog.policies.map(policy => <option key={policy.id} value={policy.id}>{policy.name}</option>)}</select><p className="field-description">{catalog.policies.find(p => p.id === config.controller_id)?.description}</p></div>
              ...
              <div className="field-group"><div className="group-label"><span>Introduce a disturbance</span><span className="fault-mini"><span />Actuator fault</span></div><div className="field"><label htmlFor="fault-asset">Outlet asset</label><select id="fault-asset" value={config.fault_asset} onChange={e => updateConfig('fault_asset', e.target.value)}>{selectedModel?.asset_ids.map(asset => <option key={asset} value={asset}>{asset}</option>)}</select></div><div className="field"><label htmlFor="fault-kind">Fault behavior</label><select id="fault-kind" value={config.fault_kind} onChange={e => updateConfig('fault_kind', e.target.value)}><option value="stuck_closed">Stuck closed</option></select></div><div className="field-pair"><div className="field"><label htmlFor="fault-start">Starts at</label><div className="input-with-unit"><input id="fault-start" type="number" required min="0" max={Math.max(0, (selectedModel?.duration_hours ?? 78) - 0.01)} step="any" value={config.start_hour} onChange={e => updateConfig('start_hour', e.target.valueAsNumber)} /><span>h</span></div></div><div className="field"><label htmlFor="fault-duration">Duration</label><div className="input-with-unit"><input id="fault-duration" type="number" required min="0.01" max={Math.max(.01, (selectedModel?.duration_hours ?? 78) - config.start_hour)} aria-invalid={config.start_hour + config.duration_hours > (selectedModel?.duration_hours ?? 78)} aria-describedby={config.start_hour + config.duration_hours > (selectedModel?.duration_hours ?? 78) ? 'fault-duration-error' : undefined} step="any" value={config.duration_hours} onChange={e => updateConfig('duration_hours', e.target.valueAsNumber)} /><span>h</span></div></div></div></div>
              {config.start_hour + config.duration_hours > (selectedModel?.duration_hours ?? 78) && <p className="field-error" id="fault-duration-error" role="alert">The disturbance must end within the {number(selectedModel?.duration_hours)} h model horizon.</p>}<div className="additional-faults">{(config.additional_faults ?? []).map((fault, index) => <div className="additional-fault" key={index}><div className="additional-fault-heading"><strong>Additional fault {index + 1}</strong><button type="button" className="icon-button" aria-label={`Remove additional fault ${index + 1}`} onClick={() => updateConfig('additional_faults', (config.additional_faults ?? []).filter((_, i) => i !== index))}><X size={13} /></button></div><div className="field"><label htmlFor={`extra-asset-${index}`}>Outlet stuck closed</label><select id={`extra-asset-${index}`} value={fault.asset} onChange={e => updateAdditionalFault(index, { asset: e.target.value })}>{selectedModel?.asset_ids.map(asset => <option key={asset} value={asset}>{asset}</option>)}</select></div><div className="field-pair"><div className="field"><label htmlFor={`extra-start-${index}`}>Starts at</label><div className="input-with-unit"><input id={`extra-start-${index}`} type="number" required min="0" max={Math.max(0, (selectedModel?.duration_hours ?? 78) - .01)} step="any" value={fault.start_hour} onChange={e => updateAdditionalFault(index, { start_hour: e.target.valueAsNumber })} /><span>h</span></div></div><div className="field"><label htmlFor={`extra-duration-${index}`}>Duration</label><div className="input-with-unit"><input id={`extra-duration-${index}`} type="number" required min=".01" max={Math.max(.01, (selectedModel?.duration_hours ?? 78) - fault.start_hour)} aria-invalid={fault.start_hour + fault.duration_hours > (selectedModel?.duration_hours ?? 78)} aria-describedby={fault.start_hour + fault.duration_hours > (selectedModel?.duration_hours ?? 78) ? `extra-duration-error-${index}` : undefined} step="any" value={fault.duration_hours} onChange={e => updateAdditionalFault(index, { duration_hours: e.target.valueAsNumber })} /><span>h</span></div></div></div>{fault.start_hour + fault.duration_hours > (selectedModel?.duration_hours ?? 78) && <p className="field-error" id={`extra-duration-error-${index}`} role="alert">This disturbance must end within the model horizon.</p>}</div>)}{(config.additional_faults?.length ?? 0) < 3 && <button type="button" className="add-fault-button" onClick={() => updateConfig('additional_faults', [...(config.additional_faults ?? []), { asset: selectedModel?.asset_ids.find(asset => asset !== config.fault_asset) ?? config.fault_asset, start_hour: config.start_hour, duration_hours: config.duration_hours }])}><Plus size={13} />Add another outlet fault</button>}</div>
              <details className="advanced-settings"><summary>Search & comparison <ChevronDown size={14} /></summary><div className="field"><label htmlFor="budget">Search budget</label><select id="budget" value={config.budget} onChange={e => updateConfig('budget', Number(e.target.value))}>{[...new Set([4, 8, 12, 20, 24, config.budget])].filter(value => value <= 24).sort((a, b) => a - b).map(value => <option key={value} value={value}>{value} simulator calls</option>)}</select></div><div className="field"><label htmlFor="fallback">Fallback policy</label><select id="fallback" value={config.fallback_controller_id} onChange={e => updateConfig('fallback_controller_id', e.target.value)}>{catalog.policies.map(policy => <option key={policy.id} value={policy.id}>{policy.name}</option>)}</select></div></details>
            </fieldset>
            <button className="button primary investigate-button" type="submit" disabled={busy || !Number.isFinite(config.threshold) || !Number.isFinite(config.start_hour) || !Number.isFinite(config.duration_hours)}>{busy ? <LoaderCircle size={17} className="spin" /> : <Play size={15} fill="currentColor" />}{busy ? 'Investigating…' : stale ? 'Run with new settings' : 'Investigate plan'}{!busy && <ArrowRight size={17} />}</button>
            <p className="run-description">Run the reference, stress the policy, then compare the evidence.</p>
      ...
      <main id="investigation" className="investigation">
        <div className="page-context"><span><span className="context-dot" />Simulation workspace</span><span className="context-right">{view ? `${view.provenance.model_name || view.model.name} / ${number(view.model.duration_hours)} h` : 'Policy resilience investigation'}</span></div>
        {error && <div className="error-banner" role="alert"><TriangleAlert size={19} /><div><strong>Experiment needs attention</strong><p>{error}</p>{!view && <p>No simulation result was produced.</p>}</div><button className="text-button" onClick={() => catalog ? void startExperiment() : setLoadKey(key => key + 1)} disabled={busy}>Retry <RotateCcw size={14} /></button><button className="icon-button" aria-label="Dismiss error" onClick={() => setError('')}><X size={16} /></button></div>}
        ...
        {view ? <>
          <section className={`finding-section ${view.finding.status}`} aria-labelledby="result-title">
            ...
            <div className="outcome-strip">
              ...
              <div className="outcome-support"><span>Tested coverage</span><strong>{number(view.search.evaluated, 0)}<small>calls</small></strong><p>{number(view.search.budget, 0)}-call budget</p></div>
          ...
          <div className="investigation-details">
            ...
            <section className="witness-panel panel" aria-labelledby="witness-title"><div className="section-heading"><div><div className="section-index"><Activity size={15} />Failure investigation</div><h2 id="witness-title">Conditions behind the result.</h2></div><span className="small-badge" title={view.search.method}>{view.search.method.includes('subset') ? 'Subset search' : human(view.search.method)}</span></div>
              <div className="fault-record"><span className="fault-record-icon"><Settings2 size={21} /></span><div><span className="fault-record-label">Declared disturbance</span><h3>{human(view.config.fault_kind)} <span>at {view.config.fault_asset}</span></h3><p>Starts at {number(view.config.start_hour)} h · lasts {number(view.config.duration_hours)} h</p></div></div>
              {!!view.config.additional_faults?.length && <div className="additional-fault-summary">{view.config.additional_faults.map((fault, index) => <span key={index}>Also stuck closed: {fault.asset} · {number(fault.start_hour)}–{number(fault.start_hour + fault.duration_hours)} h</span>)}</div>}
