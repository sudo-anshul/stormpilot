  ...
  return <div className="app-shell">
    ...
    <div className="workspace-layout">
      <aside className={`setup-rail ${setupOpen ? 'is-open' : ''}`} aria-label="Experiment settings">
        ...
        <div id="setup-content" className="setup-content">
          ...
          {booting && !config ? <div className="setup-loading"><LoaderCircle className="spin" size={20} /><p>Loading models and policies…</p></div> : config && catalog ? <form id="experiment-form" onSubmit={event => { event.preventDefault(); void startExperiment(); }}>
            ...
            <button className="button primary investigate-button" type="submit" disabled={busy || !Number.isFinite(config.threshold) || !Number.isFinite(config.start_hour) || !Number.isFinite(config.duration_hours)}>{busy ? <LoaderCircle size={17} className="spin" /> : <Play size={15} fill="currentColor" />}{busy ? 'Running experiment…' : config.mode === 'discover' ? 'Discover a compound failure' : stale ? 'Run with new settings' : 'Test defined faults'}{!busy && <ArrowRight size={17} />}</button>
      ...
      <main id="investigation" className="investigation">
        <div className="workspace-purpose"><strong>Find interacting failures. Test candidate responses.</strong><p>Bring a supported stormwater model, investigate a declared fault envelope, and keep the evidence behind every decision.</p></div>
        <nav className="workflow-nav" aria-label="Investigation workflow"><button onClick={() => { setSetupOpen(true); requestAnimationFrame(() => document.getElementById('setup-content')?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth', block: 'start' })); }}><span>1</span>Define test</button><a href="#result-title"><span>2</span>Discover failure</a><a href={responseAvailable ? '#response-search' : undefined} aria-disabled={!responseAvailable} tabIndex={responseAvailable ? 0 : -1}><span>3</span>Find response</a><a href={view ? '#robustness-evaluation' : undefined} aria-disabled={!view} tabIndex={view ? 0 : -1}><span>4</span>Evaluate</a></nav>
